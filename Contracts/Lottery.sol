...
contract Lottery{
    ...
    function enter() public payable //when we expect to recive some ether when calling function we make it payable
    {
    ...
    function random() private view returns (uint) //none can access thi just fetch value
    {
        return uint(keccak256(block.difficulty, now, players)); //keccak256 or sha3 is the same. sha3 is like class and keccak256 is one of the object. It returns a hash so convert it into int number
